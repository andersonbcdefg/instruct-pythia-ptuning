...
def get_model_and_tokenizer(
    ...
    pretrained_model = AutoModelForCausalLM.from_pretrained(model_name, load_in_8bit=load_in_8bit, device_map="auto")
    pretrained_model.gradient_checkpointing_enable() # i'm not actually sure if this is necessary

    model = AdapterModel(pretrained_model, max_len=max_seq_len, adapter_size=adapter_size, adapter_dropout=adapter_dropout)
...
def tokenize_inputs(examples, tokenizer, max_length, adapter_size):
    ...
    for i, (prompt, response) in enumerate(zip(examples["prompt"], examples["response"])):
        ...
        input_tokens = tokenizer(prompt, return_tensors="pt").input_ids.view(-1) # (prompt_len, )
        target_tokens = tokenizer(response, return_tensors="pt")["input_ids"].view(-1) # (response_len, )
        combined = torch.cat([input_tokens, newline_token, target_tokens])
        
        # get input ids
        input_ids = torch.full((max_input_length,), tokenizer.pad_token_id)
        input_ids[0:min(combined.numel(), max_input_length)] = combined[:max_input_length] # if length is less than max_input_length, the first pad token will be treated as eos
        ...
        tgt_start = input_tokens.numel() + adapter_size # + 1 for the newline, - 1 to shift left by 1
