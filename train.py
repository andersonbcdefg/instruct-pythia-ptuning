...
def train(config: Config):
    ...
    tokenize_partial = functools.partial(tokenize_inputs, tokenizer=tokenizer, max_length=config.max_seq_len, adapter_size=config.adapter_size)
    ...
    assert config.batch_size % config.microbatch_size == 0, "batch size must be divisible by microbatch size"
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        ...
        total_steps=config.num_epochs * (len(dataloader) + 10), # just to make sure scheduler doesn't run out if some off-by-one errors/restarts 
        pct_start=0.05, 
        div_factor=1000, 
        final_div_factor=25,
    ...
    for epoch in range(config.num_epochs):
        for batch in dataloader:
            ...
            with torch.cuda.amp.autocast():
                ...
                outputs = rearrange(outputs, "b l v -> (b l) v") # bsz, seq_len, vocab_size
            ...
            if micro_batches == accum_iters:
                ...
                if (total_steps + 1) % config.print_every == 0:
                    print(f"Step {total_steps + 1}: Loss = {running_loss / config.print_every:.4f}")
                    running_loss = 0
